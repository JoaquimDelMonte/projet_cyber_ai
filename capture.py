...
##################################################
    ...
    # Charger les noms des colonnes utilisées pour entraîner le scaler
    scaler_columns = [
        ...
        for packet in capture.sniff_continuously():
            ...
            for key, flow in flows.items():
                if now - flow['end_time'] > 10:
                    ...
                    # Aligner les colonnes de stats avec celles du scaler
                    try:
                        numerical_features = [stats[col] for col in scaler_columns]
                        scaled_features = scaler.transform([numerical_features])[0]
                        for i, col in enumerate(scaler_columns):
                            stats[col] = scaled_features[i]
                    except Exception as e:
